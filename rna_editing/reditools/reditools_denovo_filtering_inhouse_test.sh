...
reditools_path=/mnt/dellfs/projects/RNA_editing/analysis_inhouse/reditools_denovo
out_path=/mnt/dellfs/projects/RNA_editing/analysis_inhouse/reditools_denovo_filtering_inhouse
analysis_id="W066T01O4"
somatic_maf=/mnt/dellfs/projects/gc_multiregion/DNA/analysis_updating/cohort/maf/cohort.final.maf
...
splicesites_gtf_file=/mnt/dellfs/projects/RNA_editing/rna_editing_pipeline_sn/resources/Gencode_annotation/gencode.v30lift37.splicesites.sorted.gtf.gz
...
# summary
echo -e "raw\tquality\trmsk\tsnp\tsplicesite\tsomatic\tfinal\talu\tnonalu\tA_I\tREDIportal" >$out_path/${analysis_id}/${analysis_id}_filtering_summary.txt
...
alu=`wc -l ${result}.editing.alu|cut -d' ' -f1`
nonalu=`wc -l ${result}.editing.nonalu|cut -d' ' -f1`
A_I=`grep 'AG\|TC' ${result}.editing|wc -l|cut -d' ' -f1`
rediportal=`cut -f 19 ${result}.editing|grep 'ed'|wc -l|cut -d' ' -f1`
echo -e "$raw\t$quality\t$rmsk\t$snp\t$splicesite\t$somatic\t$final\t$alu\t$nonalu\t$A_I\t$rediportal" >>$out_path/${analysis_id}/${analysis_id}_filtering_summary.txt
