...
cat $out_path/alu $out_path/nonalu $out_path/${analysis_id}/nonrep > $out_path/${analysis_id}/alu-nonalu-nonrep
...
total_Alu=`wc -l ${result}.rmsk.snp.alu|cut -d' ' -f1`
total_noAlu=`wc -l $out_path/${analysis_id}/nonalu|cut -d' ' -f1`
A_I_Alu=`grep 'AG\|TC' ${result}.rmsk.snp.alu|wc -l|cut -d' ' -f1`
C_U_Alu=`grep 'GA\|CT' ${result}.rmsk.snp.alu|wc -l|cut -d' ' -f1`
A_I_noAlu=`grep 'AG\|TC' ${result}.rmsk.snp.nonalu|wc -l|cut -d' ' -f1`
C_U_noAlu=`grep 'GA\|CT' ${result}.rmsk.snp.nonalu|wc -l|cut -d' ' -f1`
A_to_I=`expr $A_I_Alu + $A_I_noAlu`
C_to_U=`expr $C_U_Alu + $C_U_noAlu`
REDIportal_Alu=`grep -w ed $out_path/${analysis_id}/alu|wc -l|cut -d' ' -f1`
REDIportal_noAlu=`grep -w ed $out_path/${analysis_id}/nonalu|wc -l|cut -d' ' -f1`
total_REDIportal=`expr $REDIportal_Alu + $REDIportal_noAlu`
A_I_REDIportal_Alu=`grep -w ed $out_path/${analysis_id}/alu|grep 'AG\|TC'|wc -l|cut -d' ' -f1`
A_I_REDIportal_noAlu=`grep -w ed $out_path/${analysis_id}/nonalu|grep 'GA\|CT'|wc -l|cut -d' ' -f1`
A_I_REDIportal=`expr $A_I_REDIportal_Alu + $A_I_REDIportal_noAlu`
total=`expr $total_Alu + $total_noAlu`
echo -e "$total\t$A_to_I\t$C_to_U\t$total_Alu\t$total_REDIportal\t$A_I_Alu\t$A_I_REDIportal" >>$out_path/${analysis_id}/${analysis_id}_filtered_anno_summary.txt
