...
python /REDItools/accessory/selectPositions.py -i $result -d -1 -c 4 -v 3 -f 0.1 -r -o ${result}.quality.sel
...
python /REDItools/accessory/selectPositions.py -i ${result}.editing.nonalu -d -1 -c 20 -v 5 -f 0.1 -r -o ${result}.editing.nonalu.sel

cat ${result}.editing.alu ${result}.editing.nonalu.sel >${result}.editing.strict
...
somatic=`cut -f 17 ${result}.quality.sel.rmsk.snp.splicesite.somatic.rediportal|grep 'somatic'|wc -l|cut -d' ' -f1`
...
echo -e "raw\tquality\trmsk\tsnp\tsplicesite\tsomatic\tfinal\tA_to_I\tC_to_U\ttotal_alu\ttotal_rediportal\tA_I_Alu\tA_I_REDIportal" >$out_path/${analysis_id}/${analysis_id}_filtering_summary_strict.txt
raw=`wc -l $in_file|cut -d' ' -f1`
quality=`wc -l $result|cut -d' ' -f1`
rmsk=`cut -f 11 ${result}.quality.sel.rmsk.snp.splicesite.somatic.rediportal|grep 'Simple_repeat\|Low_complexity'|wc -l|cut -d' ' -f1`
snp=`cut -f 13 ${result}.quality.sel.rmsk.snp.splicesite.somatic.rediportal|grep 'snp'|wc -l|cut -d' ' -f1`
splicesite=`cut -f 15 ${result}.quality.sel.rmsk.snp.splicesite.somatic.rediportal|grep 'splicesite'|wc -l|cut -d' ' -f1`
somatic=`cut -f 17 ${result}.quality.sel.rmsk.snp.splicesite.somatic.rediportal|grep 'somatic'|wc -l|cut -d' ' -f1`
final=`wc -l ${result}.editing.strict|cut -d' ' -f1`
A_to_I=`grep 'AG\|TC' ${result}.editing.strict|wc -l|cut -d' ' -f1`
C_to_U=`grep 'GA\|CT' ${result}.editing.strict|wc -l|cut -d' ' -f1`
total_alu=`wc -l ${result}.editing.alu|cut -d' ' -f1`
total_rediportal=`cut -f 19 ${result}.editing.strict|grep 'ed'|wc -l|cut -d' ' -f1`
A_I_Alu=`grep 'AG\|TC' ${result}.editing.alu|wc -l|cut -d' ' -f1`
A_I_REDIportal=`cut -f 8,19 ${result}.editing.strict|grep 'ed'|grep 'AG\|TC'|wc -l|cut -d' ' -f1`
echo -e "$raw\t$quality\t$rmsk\t$snp\t$splicesite\t$somatic\t$final\t$A_to_I\t$C_to_U\t$total_alu\t$total_rediportal\t$A_I_Alu\t$A_I_REDIportal" >>$out_path/${analysis_id}/${analysis_id}_filtering_summary_strict.txt
