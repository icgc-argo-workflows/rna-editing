...
if(ARG_MODE){
  ...
  snp_position_file = args[5]
  splicesites_pos_file = args[6]
  rmsk_region_file = args[7]
  TCGA_somatic_file = args[8]
}else{
...
# Functions -------------------------------------------------------
read.file <- function(file,startRow=1,header=TRUE, fread=FALSE, comment.char="#",stringsAsFactors = FALSE) {
    if(!file.exists(file)) {
        warning("File was not found!")
    } else {
        if(fread) {
            y <- data.frame(data.table::fread(input = file, sep = "\t", stringsAsFactors = FALSE, verbose = FALSE, 
            data.table = TRUE, showProgress = TRUE, header = TRUE, fill = TRUE, skip = 0))
        } else {
            if(grepl("\\.xlsx$", file)) {
                require(openxlsx)
                y <- openxlsx::read.xlsx(file,colNames = TRUE,startRow = startRow,check.names=FALSE)
            } else if(grepl("\\.csv$", file)) {
                y <- read.csv(file,header = header, check.names=FALSE, fill=TRUE, comment.char="")
            } else if(grepl("\\.maf$", file)) {
                y <- data.frame(data.table::fread(input = file, sep = "\t", stringsAsFactors = FALSE, verbose = FALSE, 
                data.table = TRUE, showProgress = TRUE, header = TRUE, fill = TRUE, skip = "Hugo_Symbol"))
            } else {
                y<-read.delim(file,check.names=FALSE,header=header,fill=TRUE, comment.char=comment.char, stringsAsFactors = stringsAsFactors)
            }
        }
        return(y)
    }
...
if(somatic_maf_file == "TCGA"){
  somatic_file = "/resources/mc3_somatic_snp_cufoff2.txt"
...
write.table(anno_vcf, file = paste0(analysisid,"_quality_anno.vcf"), quote=F, col.names = TRUE, row.names = FALSE, sep="\t")
...
try({anno_vcf[index %in% snp_position,]$FILTER = paste0(anno_vcf[index %in% snp_position,]$FILTER,"dbSNP;")})
...
try({anno_vcf[index %in% splicesites_pos_index,]$FILTER = paste0(anno_vcf[index %in% splicesites_pos_index,]$FILTER,"SPLICE;")})
...
write.table(anno_vcf, file = paste0(analysisid,"_position_anno.vcf"), quote=F, col.names = TRUE, row.names = FALSE, sep="\t")

# Final vcf ---------------------------------------------------------------
filtered_vcf=anno_vcf[anno_vcf$FILTER=="",]
write.table(filtered_vcf, file=paste0(analysisid,"_filtered.vcf"), quote=F, col.names = TRUE, row.names = FALSE, sep="\t")
...
write.table(summary, file=paste0(analysisid,"_filtering_summary.txt"), quote=F, col.names = TRUE, row.names = FALSE, sep="\t")
